...
async function clearQuests() {
  ...
  try {
    ...
    const [questsBefore] = await connection.execute('SELECT COUNT(*) as count FROM quests');
    const [userQuestsBefore] = await connection.execute('SELECT COUNT(*) as count FROM user_quests');
    
    console.log(`- Quests: ${(questsBefore as any[])[0].count}`);
    console.log(`- User quest progress: ${(userQuestsBefore as any[])[0].count}`);
    ...
    const [questsAfter] = await connection.execute('SELECT COUNT(*) as count FROM quests');
    const [userQuestsAfter] = await connection.execute('SELECT COUNT(*) as count FROM user_quests');
    
    console.log(`- Quests: ${(questsAfter as any[])[0].count}`);
    console.log(`- User quest progress: ${(userQuestsAfter as any[])[0].count}`);

    console.log('\n✨ All quest data has been successfully cleared!');
    console.log('📱 The frontend quest tab should now show no quests when refreshed.');
